...
export default class FlatListHome extends React.Component {
    ...
    {
        console.log(page1)
        HttpUtils.get(NetUrl.BaseUrl+this.props.id+NetUrl.Page_Size+page1).then(result=>{
            let data=result.results//获取json 数据并存在data数组中
            let dataArray=[]//这是创建该数组，目的放存在key值的数据，就不会报黄灯了
            let i=itemNo
            ...
            this.setState({
                news:this.state.news.concat( dataArray),
                isLoading: false,
                showFoot:foot,
                isRefreshing:false,
            })
